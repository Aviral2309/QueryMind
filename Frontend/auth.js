...
async function register() {
  ...
  try {
    ...
    if (res.ok) { setToken(data.token); setUser(data.user); hideAuthModal(); onLoginSuccess(data.user); }
    else errorEl.textContent = data.error || "Registration failed.";
  } catch (e) { errorEl.textContent = "Could not connect to server."; }
...
function handleOAuthCallback() {
  ...
  if (error) { showAuthModal("login"); document.getElementById("login-error").textContent = `Google login failed: ${error}`; return; }
  if (token) {
    setToken(token);
    fetch(`${API}/auth/me`, { headers: { "Authorization": `Bearer ${token}` } })
      .then(r => r.json())
      .then(user => { setUser(user); onLoginSuccess(user); window.history.replaceState({}, document.title, window.location.pathname); });
  }
}
...
function resetAppState() {
  document.getElementById("chat-messages").innerHTML = `
    <div class="chat-welcome" id="chat-welcome">
      ...
      <p>Connect a database on the left, then type your question below.</p>
      <div class="example-chips" id="suggestion-chips"></div>
    </div>`;
...
async function initAuth() {
  handleOAuthCallback();
  if (!isLoggedIn()) { document.getElementById("landing-screen").classList.add("visible"); return; }
  try {
    const res = await fetch(`${API}/auth/me`, { headers: { "Authorization": `Bearer ${getToken()}` } });
    if (res.ok) { const user = await res.json(); setUser(user); onLoginSuccess(user); }
    else { clearToken(); document.getElementById("landing-screen").classList.add("visible"); }
  } catch (e) { document.getElementById("landing-screen").classList.add("visible"); }
}